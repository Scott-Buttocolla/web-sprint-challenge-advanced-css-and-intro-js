...
function changeSpelling(artists, index, name){
  artists[index].name = name;
  return artists; 
}
console.log(changeSpelling(artists, 9, 'Vincent Van Gogh'));
...
function get20s(yearsBornDied){
  for( let i = 0; i < artists.length; i++){
    let tempArtists= artists[i].years.split('-')
    if( tempArtists[0] > 1900 && tempArtists[0] < 2000){
...
console.log(addArtist(artists, 20, 'Scott', '1984 - current day', 'Abstract Realism', 'American', 'loreum ipsom' ));
...
function lotsOfArt(artists, num1){
  let newarr=[];
  for(let i = 0; i < artists.length; i++){
    if(artists[i].includes(artists.paintings > 100)){
